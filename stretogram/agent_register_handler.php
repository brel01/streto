...
function sendMail($fname, $uname, $uemail){
  global $papa_dbh;
       ...
       if(isset($resp['fname'])){
            if(strlen($resp['fname']) > 2){
                if(strlen($resp['uname']) > 0){
                    if(strlen($resp['uemail'])){
                        if(filter_var($resp['uemail'],FILTER_VALIDATE_EMAIL)){
                            if(strlen($resp['contact']) > 9){
                                $qy = $papa_dbh->prepare("SELECT id FROM users WHERE useremail=?");
                                $qy->execute([$resp['uemail']]);
                                if($qy->rowCount() == 0){
                                    ...
                                    if($qyy->rowCount() == 0){
                                        ...
                                        if($qyyy->rowCount() == 0){
                                            if(strlen($resp['password']) > 5){
                                                ...
                                                $pepper_value = $fullname.$cac.$date.$contact.$username;
                                                $pepper = str_shuffle($pepper_value);
                                                $pwd = $resp['password'];
                                                $pwd_p = hash_hmac("sha256", $pwd, $pepper);
                                                $password = password_hash($pwd_p, PASSWORD_DEFAULT);
                                                $query = $papa_dbh->prepare("INSERT INTO users (user_id, fullname, username, useremail, contact, country, state, password, reg_date, pepper) VALUE (?,?,?,?,?,?,?,?,?,?) ");
                                                $query->execute([$unique_id, $fullname, $username, $useremail, $contact, $country, $state, $password, $reg_date, $pepper]);
                                                if($query){
                                                    // $d_array = ["fullname" => $fullname, "username"=>$username, "useremail"=>$useremail];
                                                    $send = sendMail( $fullname, $username, $useremail );
                                                    $error = "success";
